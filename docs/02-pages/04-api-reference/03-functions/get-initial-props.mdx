...
- `getInitialProps` can only be exported from page files (e.g., `pages/index.tsx`, `pages/blog/[slug].tsx`), not from regular components. To fetch data at the component level, consider exploring the [App Router](/docs/app/getting-started/fetching-data).
